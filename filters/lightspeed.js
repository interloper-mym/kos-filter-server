/* ============================================================
   ...
   File: filters/lightspeed.js
   Filter: Lightspeed (production-gc.lsfilter.com WebSocket)
   Fixed: `blocked` is now correctly !(Allow == 1)
   ============================================================ */
  ...
  for (let i = 0; i < lightspeedjson.length; i++) {
    if (lightspeedjson[i]["CategoryNumber"] == num) {
      const allowed =
        ...
        lightspeedjson[i]["Allow"] === "1";
      return [lightspeedjson[i]["CategoryName"], !allowed];
    }
  }
...
async function lightspeed(url) {
  return new Promise((resolve, reject) => {
    ...
    ws.on("message", (msg) => {
      ...
      resolve(categoryy ? categoryy : ["Uncategorized", true]);
    });
